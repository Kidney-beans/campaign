<template>
  <div id="music">
    ...
    <div :class="{ 'music-box': true, 'music-box-far': bullet }">
      ...
      <div
        @mouseleave="shouldShowBar = false"
        :class="{ showbar: true, 'showbar-hidden': !shouldShowBar }"
      >
        <!-- 音乐列表 -->
        <div class="music-List-box">
          ...
          >
            ...
            <div>{{ item.name }}</div>
            <img
              src="~assets/music/play1.png"
              @click="musPlay(item,index)"
            />
      ...
      <input
        ...
        value="{musicSeekValue}"
      />
      <!-- 搜索按钮 -->
      <div class="music-seek-img" @click="musicSeekBtn">
        <img src="~assets/music/music-seek.png" />
      </div>
      <!--音乐搜索列表 -->
      ...
      <div class="progress-bar">
        ...
        <span
          ><img
            src="~assets/music/last.png"
            class="music-btn-last some-btns"
            @click="preMusic"
        /></span>
        <span id="musicPause" v-show="!pause" @click="musicPause">
          <img
            src="~assets/music/pause.png"
            class="some-btns music-btn-position music-btn-play"
            id="music-play"
          />
        </span>
        <span id="musicPlay" v-show="pause" @click="musicPlay">
          <img
            ...
            class="some-btns music-btn-position music-btn-pause"
            id="music-pause"
          />
          <!-- 注意！注意！播放完歌后按钮没变暂停 -->
        </span>
        <span
          ><img
            src="~assets/music/last.png"
            class="some-btns music-btn-next"
            @click="nextMusic"
        /></span>
        <volumn-control
          side-length="4vh"
        ...
        <div class="music-porgre-border" @click="adaptMusicTime" real="p">
          <span
            class="music-porgre-solid"
            real="s"
            :style="{
              left: -100 + (currentMusicTime / currentDuration) * 100 + '%',
            }"
          ></span>
        </div>
    ...
    <div class="supplement-components">
      <el-dialog
        ...
        width="50%"
        top="17vh"
        custom-class="search-result-container"
      >
        <div class="cuntent-container">
          <!-- 搜索音乐结果 -->
          <div
            class="musicPhotoMaxBox"
            v-for="(item, index) in musicSeekArr"
            :key="item.url"
          >
            <div class="musicPhotoBox">
              <img :src="item.photourl" @mouseenter="PhotoShadeShow(index)" />
            </div>
            <div
              class="musicPhotoBox-Shade"
              v-show="index == currentPhotoIndex"
              @mouseleave="PhotoShadeHidden(index)"
            >
              <!-- 搜索音乐播放 -->
    ...
    return {
      ...
      musicData: [
        ...
        },
        {
          ...
          name: "心淡",
          url:
            "http://m7.music.126.net/20201129220005/51015f9bc6e39c6b0d0a4df7d07c440f/ymusic/e056/1f4c/9434/a1e451b55726a79905c28d90a892a7d9.mp3",
        },
        {
          author: "容祖儿",
          name: "痛爱",
      ...
      musicSeekArr: [
        {
          photourl:
            "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1606665580394&di=b9f72d368d4e74097429ee373b6aa530&imgtype=0&src=http%3A%2F%2Fi1.hdslb.com%2Fbfs%2Farchive%2F6d44a33ef2ba96351a310bc33f9bffe4fbecdeda.jpg",
          author: "谢安琪",
          name: "喜帖街",
        ...
        {
          photourl:
            "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1606666362258&di=6efab5420325693fa38e07b0f86e2f4d&imgtype=0&src=http%3A%2F%2Fpic.qqtn.com%2Fup%2F2017-5%2F2017531014305530.png",
          author: "容祖儿",
          name: "傻女",
  ...
  methods: {
    timeUpdate(e) {
      ...
      if (srcEle.duration - srcEle.currentTime <= 0) {
        this.nextMusic(this.musicData[this.currentMusicIndex+1],this.currentMusicIndex+1)
      }
      // console.log("已更改：");
      // console.log(srcEle);
    ...
    adaptMusicTime(e) {
      ...
      if (real == "p") {
        ...
        this.currentMusicTime =
          (positionX / parentWidth) * this.currentDuration;
        this.musicPlay();
        this.setMusicTime();
      } else {
        let hiddenLength =
          -(-1 + this.currentMusicTime / this.currentDuration) *
    ...
    musPlay(music,index) {
      ...
      this.musicPause();
      this.currentMusicSrc = music.url;
      this.currentMusicTime = 0;
      this.setMusicTime();
      this.musicPlay();
      this.currentMusicIndex = index;
      this.currentMusicId = music.id;
      this.bullets = music.bullets
    ...
    preMusic() {
      ...
      this.musPlay(
        this.musicData[this.currentMusicIndex],this.currentMusicIndex
      );
    },
    nextMusic() {
      console.log("下一首音乐");
